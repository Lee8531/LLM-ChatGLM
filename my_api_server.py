...
model_dir2 = snapshot_download("BAAI/bge-m3") #这里如果报错找不到，可以切换一个模型

# set LLM path
MODEL_PATH = os.environ.get('MODEL_PATH', model_dir)
TOKENIZER_PATH = os.environ.get("TOKENIZER_PATH", model_dir)

# set Embedding Model path
EMBEDDING_PATH = os.environ.get('EMBEDDING_PATH', model_dir2)
...
if __name__ == "__main__":
    ...
    model = AutoModel.from_pretrained(MODEL_PATH, trust_remote_code=True, device_map="auto").npu().eval()

    # load Embedding
    embedding_model = SentenceTransformer(EMBEDDING_PATH, device=torch_device)
